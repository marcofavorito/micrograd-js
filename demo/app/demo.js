...
function loss(X, y, model, lr, alpha, batch_size) {
  ...
  if (batch_size != null) {
    ...
    X_b = indexes.map(function (e) {
      return X[e];
    });
    y_b = indexes.map(function (e) {
      return y[e];
    });
  } else {
    X_b = X;
  ...
  var inputs = X_b.map(function (row) {
    return row.map(function (x) {
      return microgradjs.ensureValue(x);
    });
  });
...
function do_epoch(k, model, X, y, lr, alpha, batch_size) {
  // # forward
  var _a = loss(X, y, model, lr, alpha, batch_size),
    data_loss = _a[0],
    reg_loss = _a[1],
    total_loss = _a[2],
    acc = _a[3];
